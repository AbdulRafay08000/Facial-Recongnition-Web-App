...
def add():
    ...
    if os.path.isdir(userimagefolder):
        return render_template('home.html', mess='User with the same ID already exists. Please choose a different ID.')
    ...
    # Train the model after adding a new user
    print('Training Model')
    train_model()
    
    # Reload attendance data and render the home template
    names, rolls, times, l = extract_attendance()
    return render_template('home.html', names=names, rolls=rolls, times=times, l=l, totalreg=totalreg(), datetoday2=datetoday2)
            ...
            with open(f'Attendance/Attendance-{datetoday}.csv', 'r') as f:
                ...
                for line in lines:
                    if f"{manual_username},{manual_userid}" in line:
                        ...
                        names, rolls, times, l = extract_attendance()  # Reload attendance data
                        return render_template('home.html', names=names, rolls=rolls, times=times, l=l, totalreg=totalreg(), datetoday2=datetoday2, error=error_message)
            
            # Append manual attendance to CSV file
            with open(f'Attendance/Attendance-{datetoday}.csv', 'a') as f:
                f.write(f'\n{manual_username},{manual_userid},{current_time}')
            
            # Reload attendance data and redirect to home page after submitting manual attendance
            names, rolls, times, l = extract_attendance()
            return render_template('home.html', names=names, rolls=rolls, times=times, l=l, totalreg=totalreg(), datetoday2=datetoday2)
        else:
            # Render the home template with an error message
